...
% A consistent heuristic has f(n) always decreasing, where f(n) = g(n) (tracking cost) + h(n) (heuristic)

% Example heuristics: manhatten distance, straight line distance (as crow flies), number of misplaced tiles, etc.

% If one heuristic always gives larger than another it dominates the other and is better. This is because it is closer to the actual cost.
